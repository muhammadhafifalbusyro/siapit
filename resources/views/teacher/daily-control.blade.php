...
<head>
    ...
    <title>Input Kontrol Harian - SIAPIT</title>
    ...
    <script>
        ...
        function toggleInputType(val) {
            const label = document.getElementById('kkm-label');
            const kkmInput = document.querySelector('input[name="kkm"]');
            const activeTypeLabel = document.getElementById('active-type-label');
            
            if (val === 'checklist') {
                if (label) label.innerText = 'Target Minimal Hadir (Hari)';
                if (kkmInput) { kkmInput.value = 20; kkmInput.max = 31; }
                if (activeTypeLabel) activeTypeLabel.innerText = 'Ceklis (Ya/Tidak)';
            } else if (val === 'counter') {
                if (label) label.innerText = 'Target Kuantitas Mingguan';
                if (kkmInput) { kkmInput.value = 3; kkmInput.max = 999; }
                if (activeTypeLabel) activeTypeLabel.innerText = 'Target Kuantitas / Counter';
            } else {
                if (label) label.innerText = 'Target Minimal KKM (0-100)';
                if (kkmInput) { kkmInput.value = 80; kkmInput.max = 100; }
                if (activeTypeLabel) activeTypeLabel.innerText = 'Nilai Angka (0-100)';
            }
            
            // Recalculate grid spreadsheet elements
    ...
    <div class="dashboard-wrapper">
            ...
            <div class="sidebar-footer">
                <div class="user-profile-sm">
                    ...
                    <div class="user-meta-sm">
                        <h4>{{ $user->name }}</h4>
                        <p>{{ $user->teacher_type ?? 'Pengajar' }}</p>
                    </div>
                </div>
        ...
        <main class="main-content">
            ...
            <header class="main-header">
                <div class="welcome-section" style="display: flex; align-items: center; gap: 1rem; width: 100%;">
                    <a href="{{ route('pengajar.matriculation.daily-control.list') }}" style="border: none; background: var(--bg-primary); box-shadow: var(--nm-flat-sm); width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--text-primary); transition: var(--transition); text-decoration: none;" onmouseover="this.style.boxShadow='var(--nm-flat-hover)'" onmouseout="this.style.boxShadow='var(--nm-flat-sm)'">
                        <i class="fa-solid fa-arrow-left"></i>
                    </a>
                    <div>
                        <h1>Kontrol Nilai Harian: {{ $classroom->name }}</h1>
                        <p>Grid kalender satu bulan penuh untuk pengisian ceklis (Senin-Jumat) dan nilai (Sabtu Tutup Buku) santri bimbingan Anda.</p>
                    </div>
                </div>
            </header>
            ...
            <!-- Filter Grid Spreadsheet Pengajar -->
            <div class="dashboard-panel" style="width: 100%; margin-bottom: 1.5rem; padding: 1.25rem 1.5rem;">
                <form method="GET" action="{{ route('pengajar.daily-control', $classroom->id) }}" style="display: flex; flex-wrap: wrap; gap: 1.5rem; align-items: flex-end;">
                    @if($activePeriod && $activePeriod->aspects->count() > 0)
                        <div style="flex: 2; min-width: 200px; display: flex; flex-direction: column; gap: 0.5rem;">
                            ...
                            <div class="input-wrapper" style="height: 42px; display: flex; align-items: center;">
                                <select name="matriculation_aspect_id" onchange="this.form.submit()" style="width: 100%; border: none; background: transparent; outline: none; font-weight: 600; color: var(--text-primary); padding: 0 1rem; height: 100%;">
                        ...
                        <div style="flex: 1.5; min-width: 150px; display: flex; flex-direction: column; gap: 0.5rem;">
            ...
            @if(!$activePeriod || !$selectedAspect)
                <div class="dashboard-panel" style="width: 100%; text-align: center; padding: 4rem 2rem; color: var(--text-secondary);">
                    Masa matrikulasi belum aktif atau belum dikonfigurasi oleh Super Admin.
                </div>
            @else
                @php
                ...
                <!-- Diagram Chart Tren Kelas Harian Pengajar -->
                @if($students->count() > 0)
                ...
                <div class="dashboard-panel" style="width: 100%;">
                    <form id="daily-control-form" method="POST" action="{{ route('pengajar.daily-control.store') }}">
                        @csrf
                        <input type="hidden" name="classroom_id" value="{{ $classroom->id }}">
                        <input type="hidden" name="matriculation_aspect_id" value="{{ $selectedAspectId }}">
                        ...
                        <div style="background: var(--bg-primary); box-shadow: var(--nm-inset-sm); padding: 1rem; border-radius: 12px; margin-bottom: 1.5rem; display: flex; flex-wrap: wrap; gap: 1.5rem; align-items: center; max-width: 500px;">
                            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                ...
                                <div class="input-wrapper" style="height: 36px; width: 150px; display: flex; align-items: center; border-radius: 8px;">
                                    <select name="input_type" style="width: 100%; border: none; background: transparent; outline: none; font-weight: 600; color: var(--text-primary); padding: 0 0.5rem; height: 100%;" onchange="toggleInputType(this.value)">
                        ...
                        <div class="table-container" style="overflow-x: auto; max-width: 100%; border-radius: 12px; box-shadow: var(--nm-inset-sm); margin-bottom: 1.5rem; background: var(--bg-primary);">
                            <table class="grid-table" style="min-width: 100%; border-collapse: collapse; white-space: nowrap;">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="{{ count($dates) + 2 }}" style="text-align: center; padding: 2rem; color: var(--text-secondary);">Kelas bimbingan Anda belum memiliki anggota.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                            ...
                                            @foreach($weeks as $wIdx => $weekDates)
                                                @foreach($weekDates as $date)
                                                    @php
                                                        ...
                                                        $dayScores = \App\Models\MatriculationScore::whereIn('matriculation_student_id', $students->pluck('id'))
                                                            ->where('matriculation_aspect_id', $selectedAspectId)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
                            ...
                            tooltip: {
                                callbacks: {
                                    ...
                                    label: function(context) {
                                        const selectTypeEl = document.querySelector('select[name="input_type"]');
                                        const isChecklist = (selectTypeEl ? selectTypeEl.value : document.querySelector('input[name="input_type"]')?.value) === 'checklist';
            ...
            function recalculateGrid() {
                const selectedType = document.querySelector('select[name="input_type"]')?.value || document.querySelector('input[name="input_type"]')?.value;
