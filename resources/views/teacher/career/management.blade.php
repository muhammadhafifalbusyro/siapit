...
<body>
    ...
    <main class="main-content">
        <header class="main-header">
            ...
            <a href="{{ route('pengajar.career.penilaian.divisi', $student->career_placement_id) }}" class="btn-submit" style="box-shadow: var(--nm-flat-sm); background: var(--bg-primary); color: var(--text-secondary);">
                <i class="fa-solid fa-arrow-left"></i> Kembali ke List Santri
            </a>
        </header>

        @if(session('success'))
            <div class="dashboard-panel" style="padding: 1rem; color: var(--accent-green); font-weight: 850; margin-bottom: 1.5rem; box-shadow: var(--nm-inset-sm); border-radius: 12px; border: 1.5px solid rgba(255,255,255,0.4); background: var(--bg-primary);">
                ...
                @if($activeTab === 'overview')
                    ...
                    <div style="display: grid; grid-template-columns: 1fr; gap: 1.5rem; margin-bottom: 2rem;">
                        @forelse($summaries as $sum)
                            <div style="background: var(--bg-primary); box-shadow: var(--nm-flat-sm); padding: 1.5rem; border-radius: 15px; border: 1.5px solid rgba(255,255,255,0.4); display: flex; justify-content: space-between; align-items: center;">
                                <div>
                                    <h4 style="font-weight: 800; font-size: 1.1rem; color: var(--accent-blue); margin-bottom: 0.25rem;">{{ $sum['context']->name }}</h4>
                    ...
                    <div style="background: var(--bg-primary); box-shadow: var(--nm-flat-sm); padding: 1.5rem; border-radius: 15px; border: 1.5px solid rgba(255,255,255,0.4); display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <h4 style="font-weight: 800; font-size: 1.1rem; color: var(--accent-green); margin-bottom: 0.25rem;">Total Penghasilan Santri</h4>
                ...
                @else
                    @if($activeTab === 'income')
                        <div style="display: grid; grid-template-columns: 1.3fr 1fr; gap: 2rem; align-items: start;">
                            <!-- List of incomes -->
                            <div>
                                <div style="background: var(--bg-primary); box-shadow: var(--nm-flat-sm); padding: 1rem; border-radius: 12px; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; border: 1.5px solid rgba(255,255,255,0.4);">
                                    ...
                                    <div class="submission-card">
                                        <div class="submission-header">
                                            ...
                                            <div style="display: flex; gap: 0.5rem; align-items: center;">
                                                ...
                                                <form method="POST" action="{{ route('pengajar.career.reports.incomes.approve', $inc->id) }}" style="margin: 0; display: inline-flex; align-items: center;">
                                                    @csrf
                                                ...
                                                <form method="POST" action="{{ route('pengajar.career.reports.incomes.destroy', $inc->id) }}" onsubmit="return confirm('Hapus data penghasilan ini?')">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <!-- Right form to Add/Edit dynamic income -->
                            <div style="background: var(--bg-primary); box-shadow: var(--nm-flat-md); padding: 1.5rem; border-radius: 15px; border: 1.5px solid rgba(255,255,255,0.4);">
                                <h3 class="panel-title" id="income-form-title"><i class="fa-solid fa-plus"></i> Input Penghasilan</h3>
                                <form id="income-form" method="POST" action="{{ route('pengajar.career.reports.incomes.store', $student->id) }}" style="display: flex; flex-direction: column; gap: 1.25rem;">
                                    @csrf
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="text" name="source" id="income_source" placeholder="Contoh: Gaji Magang PT. XYZ, Freelance Web" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="date" name="date" id="income_date" value="{{ date('Y-m-d') }}" required style="color: var(--text-primary);">
                                        </div>
                                    </div>
                    ...
                    @else
                        ...
                        <div style="display: grid; grid-template-columns: 1.3fr 1fr; gap: 2rem; align-items: start;">
                                    ...
                                    <div class="submission-card">
                                        <div class="submission-header">
                                            ...
                                            <div style="display: flex; gap: 0.5rem;">
                                                ...
                                                <form method="POST" action="{{ route('pengajar.career.reports.submissions.destroy', $sub->id) }}" onsubmit="return confirm('Hapus submisi karya ini?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn-action-sm" style="color: var(--accent-red);" title="Hapus"><i class="fa-solid fa-trash"></i></button>
                                                </form>
                                        ...
                                        <!-- Grading card form -->
                                        <form method="POST" action="{{ route('pengajar.career.reports.submissions.assess', $sub->id) }}" style="border-top: 1.5px solid rgba(255,255,255,0.4); padding-top: 1rem; display: grid; grid-template-columns: 120px 1fr 100px; gap: 1rem; align-items: flex-end;">
                                            @csrf
                            ...
                            <div style="background: var(--bg-primary); box-shadow: var(--nm-flat-md); padding: 1.5rem; border-radius: 15px; border: 1.5px solid rgba(255,255,255,0.4);">
                                ...
                                <form id="submission-form" method="POST" action="{{ route('pengajar.career.reports.submissions.store', [$student->id, $activeContext->id]) }}" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 1.25rem;">
                                    @csrf
    ...
    <script>
        ...
        function editSubmission(id, values, score, notes) {
            ...
            form.action = `/pengajar/career/reports/submissions/${id}`;
        ...
        function resetForm() {
            ...
            const form = document.getElementById('submission-form');
            form.action = "{{ route('pengajar.career.reports.submissions.store', [$student->id, $activeContext ? $activeContext->id : 0]) }}";
        ...
        function editIncome(id, amount, source, date, notes) {
            ...
            form.action = `/pengajar/career/reports/incomes/${id}`;
        ...
        function resetIncomeForm() {
            ...
            const form = document.getElementById('income-form');
            form.action = "{{ route('pengajar.career.reports.incomes.store', $student->id) }}";
