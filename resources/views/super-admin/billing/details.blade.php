...
<body>
    ...
    <main class="main-content">
        ...
        <div class="dashboard-panel">
            <!-- Filter Row -->
            <form method="GET" action="{{ route('super-admin.billing.categories.details', $category->id) }}" style="display: flex; gap: 1.5rem; flex-wrap: wrap; align-items: flex-end; margin-bottom: 2rem; padding: 1.5rem; border-radius: 20px; box-shadow: var(--nm-flat); border: 1.5px solid rgba(255,255,255,0.4); background: var(--bg-primary);">
                        ...
                        @forelse($students as $student)
                            ...
                            <tr style="{{ !$isBilled ? 'opacity: 0.5;' : '' }}">
                                ...
                                @for($i = 1; $i <= $category->installment_count; $i++)
                                    ...
                                    @endphp
                                    <td>
                                        <div style="background: var(--bg-primary); box-shadow: var(--nm-inset-sm); border-radius: 8px; padding: 0.15rem 0.25rem; width: 110px;">
                                            <input type="text" 
                                                   ...
                                                   {{ !$isBilled ? 'disabled' : '' }}
                                                   style="border: none; background: transparent; outline: none; padding: 0.25rem; font-size: 0.8rem; font-weight: 700; width: 100%;">
                                        </div>
                                    </td>
                                @endfor
                ...
                <div class="pagination-wrapper" style="display: flex; align-items: center; justify-content: center; gap: 0.75rem; padding: 1.5rem; width: 100%; border-top: 1px solid #d1d9e6; background: var(--bg-primary); margin-top: 1.5rem;">
                    ...
                    @for ($page = 1; $page <= $lastPage; $page++)
                        @if ($page == 1 || $page == $lastPage || abs($page - $currentPage) <= $sidePages)
                            ...
                            @else
                                <a href="{{ $students->appends(request()->query())->url($page) }}" class="pagination-btn" style="box-shadow: var(--nm-flat-sm); color: var(--text-primary); font-weight: 700; width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; text-decoration: none; transition: var(--transition);">{{ $page }}</a>
                            @endif
