...
<head>
    ...
    <title>Manajemen Akun Santri - SIAPIT</title>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            <div class="main-header">
                <h1>Manajemen Akun Santri</h1>
                <p>Kelola kredensial akun login calon santri yang dinyatakan DITERIMA pada Tahap Penerimaan.</p>
            </div>

            @if(session('success'))
                <div class="card-nm" style="margin-bottom: 2rem; padding: 1rem 1.5rem; background: #e6f9f1; color: var(--accent-teal); font-weight: 700; border-radius: 12px; display: flex; align-items: center; gap: 0.5rem; box-shadow: var(--nm-inset-sm);">
                    <i class="fa-solid fa-circle-check"></i> {{ session('success') }}
                </div>
            @endif

            <!-- List Panel -->
            <div class="card-nm" style="padding: 2rem;">
                <h3 style="font-family: var(--font-heading); font-size: 1.15rem; font-weight: 800; color: var(--text-primary); margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;"><i class="fa-solid fa-users-gear"></i> Daftar Akun Santri ({{ count($students) }})</h3>
                        ...
                        <tbody>
                            @forelse($students as $st)
                                <tr>
                                    ...
                                    <td><code style="font-weight: 800; color: var(--accent-blue); background: rgba(0, 132, 255, 0.08); padding: 0.2rem 0.5rem; border-radius: 4px;">{{ $st->username }}</code></td>
                                    <td>{{ $st->email }}</td>
                                    <td>{{ $st->whatsapp ?? '-' }}</td>
                                    <td><code style="font-weight: 700; color: var(--text-secondary);">santri{{ $st->username }}</code></td>
                                    <td>
                                        <form method="POST" action="{{ route('super-admin.settings.students.reset-password', $st->id) }}" onsubmit="return confirm('Reset password akun santri ini ke default?')">
                                            @csrf
                                            <button type="submit" class="btn-nm" style="color: var(--accent-red); padding: 0.4rem 0.75rem; font-size: 0.75rem; display: inline-flex; align-items: center; gap: 0.25rem;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" style="text-align: center; color: var(--text-secondary); padding: 2rem;">Belum ada akun santri baru yang tergenerate (belum ada status diterima di Tahap Penerimaan).</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
